...
class KginoItem {
  ...
  void goToDetails(BuildContext context) {
    if ([KginoProvider.flmxMovie.name, KginoProvider.flmxShow.name]
        .contains(provider)) {
      context.pushNamed(
        'flmxShowDetails',
        pathParameters: {
    ...
    if (provider == KginoProvider.ockg.name) {
      context.pushNamed('ockgDetails', pathParameters: {
        'id': id,
      });
    }

    if (provider == KginoProvider.tskg.name) {
      context.pushNamed('tskgDetails', pathParameters: {
        'id': id,
      });
    }
  }
